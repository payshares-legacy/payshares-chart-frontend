...
function handleLedger(err, obj) {
  ...
  $("#totalstellars").text(commas(parseInt(currentLedger.total_coins, 10)/1000000));
}
...
function handleAccountData(err, obj) {
  ...
  if (err && err.remote && (err.remote.error === "actNotFound" || err.remote.error === "actMalformed" )) {
    ...
    if ($.isEmptyObject(obj.account_data)) {
      ...
      if (currentCurrency == "STR") { // Change the size of the circles, and recalculate the arrows.
      ...
      if (obj.account_data.Account == focalNode) {
        //Update the STR listing on the table below. (But don't rewrite the whole table)
...
function eraseGraph() {
  ...
  $("#loading").html('<img class="loader" src="assets/images/stellarThrobber.png" style="vertical-align: middle;" /> Loading...');
}
...
function txDescription(result) {
  ...
  if (result.meta) {
    for (var i=0; i<result.meta.AffectedNodes.length; i++) {
      ...
      if (an.ModifiedNode && an.ModifiedNode.LedgerEntryType==="AccountRoot" && an.ModifiedNode.FinalFields.Account===result.Account) {
        strExpense = {before:an.ModifiedNode.PreviousFields.Balance/1000000 , after:an.ModifiedNode.FinalFields.Balance/1000000};
        break;
      }
    }
  }
  ...
  if (amount) {
    ...
    span.html(amount.currency ? commas(amount.value)+" "+amount.currency : commas(amount/1000000)+" STR");
  ...
  div.append((result.meta ? "<b>Result:</b> "+(result.meta.TransactionResult=="tesSUCCESS"?"<span>":"<span style='color:#900;'>")+result.meta.TransactionResult+"</span><br/>" : "")+
    (strExpense||strExpense===0 ? "<b>STR change:</b> "+commas(strExpense.before) + " STR &rarr; "+commas(strExpense.after)+" STR ("+(strExpense.after>=strExpense.before?"+":"&ndash;")+commas(Math.round(1000000*Math.abs(strExpense.before-strExpense.after))/1000000)+" STR)<br/>" : "")+
...
function addConnections(origin, trustLines) {
  ...
  if (currentCurrency != "STR") { // Change the size of the circle, if we needed to wait until now to figure out its balance (i.e. we're looking at a currency other than STR.)
...
var force = d3.layout.force()
  ...
  .linkStrength(function(d) {
    if (currentCurrency == "STR" || currentCurrency == d.currency) {
  ...
  $('#balanceTable').append(
    '<tr class="toprow">'+
      '<td class="circlecell"><svg width="22" height="22">'+
        '<circle cx="11" cy="11" r="11" style="fill:'+COLOR_TABLE['STR'][0][1]+';"></circle>'+
      '</svg></td>'+
      '<td class="light small mediumgray" style="width:35px;">STR</td>'+
...
function updateTransactions(address, appending) {
  ...
  if (nodes[nodeMap[address]].transactions) {
        ...
        if (tx.Account == address) {
            ...
            if (mn && LatestFields) {
              if (LatestFields.Account == address || (LatestFields.HighLimit && LatestFields.HighLimit.issuer == address) ) {
                ...
                if (type == "AccountRoot") {
                  ...
                  if (affectedBalances["STR"]) {
                    affectedBalances["STR"]+=diff;
                  } else {
                    affectedBalances["STR"]=diff;
                  }
                } else if (type == "StellarState") {
                  //console.log("Affected StellarState:", mn);
          ...
          if (affectedKeys.length == 2) {
            ...
            if (positive * negative > 0) {
              ...
              amount = affectedKeys[posKey]=="STR" ? positive : {value: positive, currency: affectedKeys[posKey]};
              secondAmount = affectedKeys[negKey]=="STR" ? -negative : {value: -negative, currency: affectedKeys[negKey]};
            }
          } else {
    ...
    if (!nodes[nodeMap[address]].transactionsFinished) { //Are there more?
      $('#transactionTable').append('<tr id="transactionThrobber"><td colspan=3 style="text-align:center; padding:10px"><img class="loader" src="assets/images/stellarThrobber.png" width=30 height=30 /></td></tr>');
...
function isLinkVisible(d) {
  return currentCurrency=="STR" || currentCurrency==d.currency
}
